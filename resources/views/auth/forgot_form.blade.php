...
@section('content')
    ...
    <div class="h-screen bg-white flex flex-col space-y-10 justify-center items-center">
        <div class="bg-white w-96 shadow-xl rounded p-5">
            ...
            <form method="POST" action="{{ route('forgot_form') }}" class="space-y-5 mt-5">
                @csrf

                <input name="email" type="text" class="w-full h-12 border rounded px-3" value="{{old('email')}}" placeholder="Email" />
                @error('email')
